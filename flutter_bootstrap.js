...
const embeddedRelease = "4ef4364afb6164cb3c2b78162eaee9adf64751de";
const isPackagedRelease = /^[0-9a-f]{40}$/i.test(embeddedRelease);
...
async function prepareRelease(release) {
  if (!isPackagedRelease) return true;
...
// Capture listeners also receive non-bubbling WebGL context-loss events.
  ...
  const isReleaseScoped =
    /^[0-9a-f]{40}$/i.test(release) && isPackagedRelease;
