!function(){try{var e="undefined"!=typeof window?window:"undefined"!=typeof global?global:"undefined"!=typeof globalThis?globalThis:"undefined"!=typeof self?self:{},n=(new e.Error).stack;n&&(e._sentryDebugIds=e._sentryDebugIds||{},e._sentryDebugIds[n]="e536393f-84d2-549c-8dd1-4362bb83706a")}catch(e){}}();
...
const embeddedRelease = "d01acc82f8c16d6e239e1375a3ac11febd916f57";
...
async function resolveRelease() {
  try {
    ...
    if (response.ok) {
      ...
      if (identity.git_sha) return String(identity.git_sha);
    }
  } catch (_) {
    // Fall through to the cached URL or the identity embedded at deploy time.
  }

  const requestedRelease = new URL(window.location.href).searchParams.get(
    "release",
  );
  if (/^[0-9a-f]{40}$/i.test(requestedRelease || "")) {
    return requestedRelease;
  }
  if (/^[0-9a-f]{40}$/i.test(embeddedRelease)) {
    return embeddedRelease;
  }
  return String(_flutter.buildConfig.engineRevision || "unknown");
}

async function prepareRelease(release) {
  if (embeddedRelease === "d01acc82f8c16d6e239e1375a3ac11febd916f57") return true;
...
async function registerDmovelServiceWorker(release) {
  if (embeddedRelease === "d01acc82f8c16d6e239e1375a3ac11febd916f57") return;
...
resolveRelease().then(async (release) => {
  ...
  for (const build of _flutter.buildConfig.builds) {
    if (build.mainJsPath) build.mainJsPath = versioned(build.mainJsPath);
    if (build.mainWasmPath) build.mainWasmPath = versioned(build.mainWasmPath);
    if (build.jsSupportRuntimePath) {
      build.jsSupportRuntimePath = versioned(build.jsSupportRuntimePath);
    }
  }

  const fallbackKey = `dmovel:webgl-cpu-fallback:${release}`;
  const recoveryKey = `dmovel:webgl-recovery:${release}`;

  function activateCpuFallback() {
...
// Capture listeners also receive non-bubbling WebGL context-loss events.
  window.addEventListener("webglcontextlost", activateCpuFallback, true);
  window.addEventListener("error", (event) => {
    const message = String(event.message || event.error || "");
    if (message.includes("LateInitializationError") &&
        (message.includes("handledContextLostEvent") || message.includes("details omitted due to --minify"))) {
      activateCpuFallback();
    }
  }, true);
  ...
  const forceCpu = localStorage.getItem(fallbackKey) === "1";
  // Flutter/Wasm reads `entrypointBaseUrl.origin` while resolving modules.
  // Keep the immutable release directory, but resolve it against the document
  // URL so GitHub Pages never receives a relative URI without a scheme.
  const isReleaseScoped =
    /^[0-9a-f]{40}$/i.test(release) &&
    embeddedRelease !== "d01acc82f8c16d6e239e1375a3ac11febd916f57";
  const releaseBase = new URL(
    isReleaseScoped ? `releases/${encodeURIComponent(release)}/` : "./",
    document.baseURI,
  ).toString();
...
//# debugId=e536393f-84d2-549c-8dd1-4362bb83706a
